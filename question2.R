...
#Save to csv
write.csv(apr_ready,'apriori_table.csv',row.names = FALSE)
...
apr_ready <- read.transactions(file='apriori_table.csv',
                               rm.duplicates = TRUE, format='basket',
                               sep=',', cols = NULL)

#apr_ready@itemInfo$labels <- gsub("\"","",apr_ready@itemInfo$labels)
complaint_rules <- apriori(apr_ready,parameter = list(sup = 0.01,conf = 0.5,target="rules"))
